...
const bodlinka = { name: 'Bodlinka: Pichlavé dobrodružství', year: '2025', csfdUrl: 'https://www.csfd.cz/sk/film/1690274-pichlave-dobrodruzstvo/prehlad/' };
...
assert.equal(getVerifiedSourceOverride(bodlinka)?.tmdbId, 1470334);
assert.equal(getVerifiedSourceOverride(bodlinka)?.imdbId, 'tt6930962');
assert.ok(getVerifiedSourceOverride(bodlinka)?.aliases?.includes('Spiked'));
